locals {
  domainControllerSubnet_nsg_rules = {
    "Allow_Bastion_Inbound" = {
      ...
      access                     = "Allow"
      destination_address_prefix = module.subnet_addresses.network_cidr_blocks[local.subnet_names.DomainControllerSubnet]
    ...
    "Allow_NTP_Udp_Inbound" = {
      ...
      priority                   = 200
      protocol                   = "Udp"
      source_address_prefix      = module.subnet_addresses.network_cidr_blocks[local.subnet_names.ComputeSubnet]
      source_port_range          = "*"
    }
